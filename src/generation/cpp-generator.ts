...
export class CppGenerator
{
    ...
    public cppHeaderFileTemplate(): string {
        ...
        result.push(`{`);
        result.push(`\tstatic void RegisterMethods();`);
    ...
    private cppAddInstanceMonoCall(methodConfig: MethodConfig): string {
        return `\tmono_add_internal_call("${this.config.className(GeneratedType.cs)}::${methodConfig.name}(int,${methodConfig.getArgs(GeneratedType.cs)})", ${this.config.className(GeneratedType.cpp)}API::${methodConfig.name});`
    }
    
    private cppAddStaticMonoCall(methodConfig: MethodConfig): string {
        return `\tmono_add_internal_call("${this.config.className(GeneratedType.cs)}::${methodConfig.name}", ${this.config.className(GeneratedType.cpp)}::${methodConfig.name});`
    }
    ...
    private cppInstanceMethod(methodConfig: MethodConfig): string {
        const result: Array<string> = new Array<string>();
        result.push(`${methodConfig.returnType(GeneratedType.cpp)} ${this.config.className(GeneratedType.cpp)}API::${methodConfig.name}(int managedInstanceId, ${methodConfig.getArgs(GeneratedType.cpp)})`)
        result.push(`{`);
        ...
        result.push(`\t${this.config.className(GeneratedType.cpp)}* nativeClassInstance = sm->GetNativeInstance(nativeClassId, managedInstanceId);`);
        result.push(`\tnativeClassInstance->${methodConfig.name}(${methodConfig.getArgs(GeneratedType.cpp)});`);
        result.push(`}`);
        return result.join('\n');
    }
    ...
    private cppStaticMethod(methodConfig: MethodConfig): string {
        const result: Array<string> = new Array<string>();
        result.push(`${methodConfig.returnType(GeneratedType.cpp)} ${this.config.className(GeneratedType.cpp)}::${methodConfig.name}(${methodConfig.getArgs(GeneratedType.cpp)})`);
        result.push(`{`);
        result.push(`\t[LOGIC]`);
        result.push(`}`);
        return result.join('\n');
